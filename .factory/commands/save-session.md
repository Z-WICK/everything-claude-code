---
description: Save current session state to a dated file in the active ECC session store so work can be resumed in a future session with full context.
---
...
Ensure the runtime-specific ECC sessions folder exists before writing. In Factory/Droid the path is `~/.factory/ecc/sessions`. In all runtimes, `scripts/lib/session-manager.js` is the source of truth for the resolved directory.

### Step 3: Write the session file

Create `[ecc-session-store]/YYYY-MM-DD-<short-id>-session.tmp`, using today's actual date and a short-id that satisfies the rules enforced by `SESSION_FILENAME_REGEX` in `session-manager.js`:

- The short-id is optional for legacy files, but new files should prefer it
- The first short-id character may be a letter, digit, or underscore
- Remaining characters may include letters, digits, underscores, and hyphens
- Do not use spaces

Valid examples: `abc123de`, `A1b2C3d4`, `frontend-worktree-1`, `ChezMoi_2`
Invalid examples: `-badstart` (leading hyphen), `test id1` (space)
...
- Use the canonical runtime-specific ECC session store resolved by `scripts/lib/session-manager.js`
