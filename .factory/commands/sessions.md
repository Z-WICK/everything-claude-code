...
Manage ECC session history across the active runtime store, including list, load, alias, and info flows.
...
function resolvePluginRoot() {
  const roots = [
    ...
    process.env['CLAUDE' + '_PLUGIN_ROOT'],
    path.join(os.homedir(), '.factory', 'plugins', 'marketplaces', 'everything-claude-code'),
    path.join(os.homedir(), '.factory', 'plugins', 'everything-claude-code'),
    path.join(os.homedir(), '.claude', 'plugins', 'everything-claude-code'),
    path.join(os.homedir(), '.claude', 'plugins', 'everything-claude-code@everything-claude-code'),
    path.join(os.homedir(), '.claude', 'plugins', 'marketplace', 'everything-claude-code')
  ].filter(Boolean);

  for (const root of roots) {
    if (fs.existsSync(path.join(root, 'scripts', 'lib', 'session-manager.js'))) {
      const normalized = root.replace(/\\/g, '/');
      if (!process.env.ECC_RUNTIME) {
        process.env.ECC_RUNTIME = normalized.includes('/.factory/plugins/') ? 'factory' : 'claude';
      }
      return root;
    }
  }

  throw new Error('Unable to resolve ECC plugin root');
}
...
for (const a of aliases) {
  aliasMap[a.sessionPath] = a.name;
  aliasMap[path.basename(a.sessionPath)] = a.name;
}
...
const aliases = [
  ...aa.getAliasesForSession(session.filename),
  ...aa.getAliasesForSession(session.sessionPath).filter(
    candidate => !aa.getAliasesForSession(session.filename).some(existing => existing.name === candidate.name)
  )
];
const prettyPath = String(session.sessionPath || '').replace(os.homedir(), '~');

console.log('Session: ' + session.filename);
console.log('Path: ' + prettyPath);
console.log('');
...
function resolvePluginRoot() {
  ...
  for (const root of roots) {
    if (fs.existsSync(path.join(root, 'scripts', 'lib', 'session-aliases.js'))) {
      const normalized = root.replace(/\\/g, '/');
      if (!process.env.ECC_RUNTIME) {
        process.env.ECC_RUNTIME = normalized.includes('/.factory/plugins/') ? 'factory' : 'claude';
      }
...
- Sessions are stored in the runtime-specific ECC session store managed by `scripts/lib/session-manager.js`
- Aliases are stored in the runtime-specific ECC alias store managed by `scripts/lib/session-aliases.js`
