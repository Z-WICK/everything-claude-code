---
description: Load the most recent session file from the active ECC session store and resume work with full context from where the last session ended.
---
...
```
/resume-session                                                      # loads most recent file in the ECC session store
/resume-session 2024-01-15                                           # loads most recent session for that date
/resume-session [ecc-session-store]/2024-01-15-session.tmp          # loads a specific legacy-format file
/resume-session [ecc-session-store]/2024-01-15-abc123de-session.tmp # loads a current short-id session file
```
...
1. Check the active ECC session store (`scripts/lib/session-manager.js` is the source of truth; Factory/Droid uses `~/.factory/ecc/sessions`)
2. Pick the most recently modified `*-session.tmp` file
3. If the folder does not exist or has no matching files, tell the user:
   ```
   No session files found in the ECC session store
...
- If it looks like a date (`YYYY-MM-DD`), search the active ECC session store for files matching
...
```
SESSION LOADED: /Users/you/path/to/ecc-session-store/2024-01-15-abc123de-session.tmp
════════════════════════════════════════════════
